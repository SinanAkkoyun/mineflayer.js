...
export let targetYaw = 0
export let targetPitch = 0
...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function tickPhysics (now) {
    ...
    if (bot.physicsEnabled && shouldUsePhysics) {
      ...
      if (typeof targetYaw === 'number' && typeof targetPitch === 'number') {
        const maxYawChange = physics.yawSpeed * PHYSICS_TIMESTEP
        const maxPitchChange = physics.pitchSpeed * PHYSICS_TIMESTEP

        const dYaw = deltaYaw(targetYaw, bot.entity.yaw)
        const dPitch = targetPitch - bot.entity.pitch

        bot.entity.yaw += math.clamp(-maxYawChange, dYaw, maxYawChange)
        bot.entity.pitch += math.clamp(-maxPitchChange, dPitch, maxPitchChange)
      }
  ...
  function updatePosition (now) {
    ...
    // === Round yaw/pitch to vanilla sensitivity step ===
    const sensitivity = conv.fromNotchianPitch(0.15) // ~0.003 radians per notch

    const rawYaw = bot.entity.yaw
    const rawPitch = bot.entity.pitch

    const yawDelta = rawYaw - lastSentYaw
    const pitchDelta = rawPitch - lastSentPitch

    // Round delta to sensitivity step
    const roundedYaw = lastSentYaw + Math.round(yawDelta / sensitivity) * sensitivity
    const roundedPitch = lastSentPitch + Math.round(pitchDelta / sensitivity) * sensitivity

    const yaw = Math.fround(conv.toNotchianYaw(roundedYaw))
    const pitch = Math.fround(conv.toNotchianPitch(roundedPitch))
    ...
    lastSentYaw = roundedYaw
    lastSentPitch = roundedPitch
